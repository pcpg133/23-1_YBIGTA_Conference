...
data = client.get_historical_klines(
    "ETHUSDT",
    "5m",
    start_str=str(datetime(2021, 12, 31, 12, 20)),
...
# set indexes.
df = df.iloc[:, 0:6]
df.columns = ["Date2", "Open2", "High2", "Low2", "Close2", "Volume2"]
...
def load_data():
    ...
    df["Date2"] = pd.to_datetime(df['Date2'], utc=True, unit='ms').dt.tz_convert("Asia/Seoul").dt.tz_localize(None)
...
def calculate_macd(df, n_fast=12, n_slow=26, n_signal=9):
    ema_fast = df['Close2'].ewm(span=n_fast, min_periods=n_fast - 1).mean()
    ema_slow = df['Close2'].ewm(span=n_slow, min_periods=n_slow - 1).mean()
    macd_line = ema_fast - ema_slow
    macd_signal = macd_line.ewm(span=n_signal, min_periods=n_signal - 1).mean()
    return macd_line, macd_signal

def calculate_cci(df, n=20):
    tp = (df['High2'] + df['Low2'] + df['Close2']) / 3
...
def get_bull_bear_engulfing(data: pd.DataFrame):
    ...
    for i in range(1, len(temp)):
        current_candle_close = temp['Close2'].iloc[i]
        current_candle_open = temp['Open2'].iloc[i]
        prev_candle_close = temp['Close2'].iloc[i-1]
        prev_candle_open = temp['Open2'].iloc[i-1]
...
def do_plot(dataframe: pd.DataFrame, title: str, position_vectors: List[PositionVector], position_alters: List[PositionAlter]):
    ...
    fig.add_trace(
        go.Scatter(x=df.index, y=df['Close2'], mode='lines', name='Close Price'),
        row=2, col=1,
    )

    # Add the RSI trace to the second subplot
    fig.add_trace(
        go.Scatter(x=df.index, y=df['RSI2'], mode='lines', name='RSI'),
        row=3, col=1
    )
...
class Agent:
    ...
    def open(self, price: float, date: datetime, is_buy=True):
        if is_buy:
            ...
            self.__send_trade_data['Datetime2'] = str(date)
            ...
            self.__send_trade_data['Amount2'] = abs(self.__asset)
            self.__send_trade_data['Total2'] = price * abs(self.__asset)
            self.__send_trade_data['PnL2'] = -1000
            send_message(json.dumps(self.__send_trade_data), 'basic-transaction2.json')
...
# BACKTESTING CODE
df = load_data()
df["RSI2"] = calc_rsi(df["Close2"], RSI_PERIOD)

# Calculate MACD
macd_line, macd_signal = calculate_macd(df)
df["MACD2"] = macd_line
df["MACD_Signal2"] = macd_signal

# Calculate CCI
cci = calculate_cci(df)
df["CCI2"] = cci

df['IsBull2'] = get_bull_bear(df)
df['IsBullishEngulfing2'] = get_bull_bear_engulfing(df)
...
for i in range(32, len(df)):
    ...
    is_overbought = df['RSI2'][i] > RSI_OVERBOUGHT
    is_oversold = df['RSI2'][i] < RSI_OVERSOLD
    macd_line = df['MACD2'][i]
    macd_signal_line = df['MACD_Signal2'][i]
    cci = df['CCI2'][i]

    if agent.is_open():
        if agent.get_asset() > 0:
            # Long position opened
            is_stop_loss_long_position = df['Close2'][i] <= (1 - STOP_LOSS_LONG_POSITIONS) * agent.get_opened_price()
            is_take_profit_long_position = df['Close2'][i] >= (1 + TAKE_PROFIT_LONG_POSITIONS) * agent.get_opened_price()

            if is_stop_loss_long_position:
                agent.close(df['Close2'][i], date)
                long_lose += 1
            ...
            is_stop_loss_short_position = df['Close2'][i] >= (1 + STOP_LOSS_SHORT_POSITIONS) * agent.get_opened_price()
            is_take_profit_short_position = df['Close2'][i] <= (1 - TAKE_PROFIT_SHORT_POSITIONS) * agent.get_opened_price()
        ...
        if (macd_line > macd_signal_line) and is_oversold and cci > -100:
            agent.open(df['Close2'][i], date, True)
        if (macd_line < macd_signal_line) and is_overbought and cci < 100:
            agent.open(df['Close2'][i], date, False)
    
    df['RoR2'].iloc[i] = ((agent.get_equity(df['Close2'][i]) / agent.get_initial_cash()) - 1) * 100

    dict_list = dataframe_to_dict(df.iloc[i].to_frame().T)
    message = json.dumps(dict_list[0])
    send_message(message, 'raw-data-basic2.json')
    producer.flush()

    agent_equity = agent.get_equity(df['Close2'][i])
...
df['Cash'] = cash_arr
result = f"Initial Cash: {agent.get_initial_cash()}, Final Equity: {agent.get_equity(df['Close2'][len(df)-1])}, Profit (%): {(((agent.get_equity(df['Close2'][len(df)-1])-1) / agent.get_initial_cash())-1)* 100}, Total Trade Count: {len(agent.get_position_alters())}"
do_plot(df, result, agent.get_position_vectors(), agent.get_position_alters())

# print report
print(f"""
    Initial Cash: {agent.get_initial_cash()}
    Final Equity: {agent.get_equity(df['Close2'][len(df)-1])}
    Profit (%): {(((agent.get_equity(df['Close2'][len(df)-1])-1) / agent.get_initial_cash())-1)* 100}
