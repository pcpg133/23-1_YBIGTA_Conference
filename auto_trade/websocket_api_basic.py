...
def get_historical_data(data):
    ...
    df['Date2'] = pd.to_datetime(df['timestamp'], utc=True, unit='ms').dt.tz_convert("Asia/Seoul").dt.tz_localize(None)
    df['@timestamp'] = df['Date2']
    df['Open2'] = df['open'].astype(float)
    df['Close2'] = df['close'].astype(float) 
    df['High2'] = df['high'].astype(float)
    df['Low2'] = df['low'].astype(float)
    df['Volume2'] = df['volume'].astype(float)

    return df[['@timestamp', 'Date2', 'Open2', 'High2', 'Low2', 'Close2', 'Volume2']]
...
def raw_to_filtered_data(data):
    ...
    df['Date2'] = pd.to_datetime(df['t'], utc=True, unit='ms').dt.tz_convert("Asia/Seoul").dt.tz_localize(None)
    df['@timestamp'] = df['Date2']
    df['Open2'] = df['o'].astype(float)
    df['Close2'] = df['c'].astype(float)
    df['High2'] = df['h'].astype(float)
    df['Low2'] = df['l'].astype(float)
    df['Volume2'] = df['v'].astype(float)
...
def get_macd(data: pd.DataFrame, n_fast=12, n_slow=26, n_signal=9):
    # 지수이동평균(EMA) 계산
    temp = data.copy()
    ema_fast = temp['Close2'].ewm(span=n_fast, min_periods=n_fast - 1).mean()
    ema_slow = temp['Close2'].ewm(span=n_slow, min_periods=n_slow - 1).mean()
...
def get_cci(data: pd.DataFrame, n=20):
    temp = data.copy()
    tp = (temp['High2'] + temp['Low2'] + temp['Close2']) / 3
...
def get_bull_bear(data: pd.DataFrame):
    temp = data.copy()
    temp['IsBull2'] = (temp['Close2'] >= temp['Open2']).astype(int)
    return temp['IsBull2']

# bear engulfing, bullish engulfing을 구하는 함수
def get_bull_bear_engulfing(data: pd.DataFrame):
    temp = data.copy()
    temp['IsBullishEngulfing2'] = -1

    for i in range(1, len(temp)):
        current_candle_close = minute_chart['Close2'].iloc[i]
        current_candle_open = minute_chart['Open2'].iloc[i]
        prev_candle_close = minute_chart['Close2'].iloc[i-1]
        prev_candle_open = minute_chart['Open2'].iloc[i-1]
        ...
        if current_candle_close > current_candle_open and prev_candle_close < prev_candle_open and prev_candle_close >= current_candle_open and prev_candle_open <= current_candle_close:
            temp.at[i, 'IsBullishEngulfing2'] = 1
            ...
            temp.at[i, 'IsBullishEngulfing2'] = 0
    
    return temp['IsBullishEngulfing2']
...
class Agent:
    ...
    def open(self, price: float, date: datetime, is_buy=True):
        # 롱 포지션 진입
        if is_buy:
            self.__send_trade_data['@timestamp'] = str(minute_chart['Date2'].iloc[-1])
            self.__send_trade_data['Datetime2'] = str(minute_chart['Date2'].iloc[-1])
            self.__send_trade_data['Position2'] = 'long'
            self.__send_trade_data['State2'] = 'open'
            self.__send_trade_data['Side2'] = 'buy'
            self.__send_trade_data['Price2'] = price
            ...
            new_balances = exchange.fetch_balance(params={"type" : "future"})
            self.__send_trade_data['Amount2'] = available_amount
            self.__send_trade_data['Total2'] = price * available_amount
            self.__send_trade_data['PnL2'] = -1000
            send_message(json.dumps(self.__send_trade_data), 'basic-transaction.json')
        ...
        # 숏 포지션 진입
            ...
            self.__send_trade_data['@timestamp'] = str(minute_chart['Date2'].iloc[-1])
            self.__send_trade_data['Datetime2'] = str(minute_chart['Date2'].iloc[-1])
            self.__send_trade_data['Position2'] = 'short'
            self.__send_trade_data['State2'] = 'open'
            self.__send_trade_data['Side2'] = 'sell'
            self.__send_trade_data['Price2'] = price
    ...
    def close(self, price: float, date: datetime):
        if self.is_open():
            ...
            if self.__is_long:
                ...
                self.__opened_price = self.__send_trade_data['Price2']
                self.__send_trade_data['@timestamp'] = str(minute_chart['Date2'].iloc[-1])
                self.__send_trade_data['Datetime2'] = str(minute_chart['Date2'].iloc[-1])
                self.__send_trade_data['Position2'] = 'long'
                self.__send_trade_data['State2'] = 'close'
                self.__send_trade_data['Side2'] = 'sell'
                self.__send_trade_data['Price2'] = price
                ...
                self.__send_trade_data['Amount2'] = available_amount
                self.__send_trade_data['Total2'] = price * available_amount
                self.__send_trade_data['PnL2'] = np.round((price - self.__opened_price) / self.__opened_price * 100, 2)
                send_message(json.dumps(self.__send_trade_data), 'basic-transaction.json')
            ...
            # 숏 포지션 청산
                ...
                self.__send_trade_data['Amount2'] = available_amount
                self.__send_trade_data['Total2'] = price * available_amount
                self.__send_trade_data['PnL2'] = np.round((self.__opened_price - price) / price * 100, 2)
                send_message(json.dumps(self.__send_trade_data), 'basic-transaction.json')
...
def handle_socket_message(msg):
    ...
    if is_candle_closed:
        ...
        minute_chart['RSI2'] = get_rsi(minute_chart['Close2'], RSI_PERIOD)
        minute_chart['MACD2'], minute_chart['MACD_Signal2'] = get_macd(minute_chart)
        minute_chart['CCI2'] = get_cci(minute_chart)
        minute_chart['IsBull2'] = get_bull_bear(minute_chart)
        minute_chart['IsBullishEngulfing2'] = get_bull_bear_engulfing(minute_chart)
        minute_chart['RoR2'] = 0.0
        minute_chart = minute_chart[-50:]

        date = minute_chart['Date2'].iloc[-1]

        is_overbought = minute_chart['RSI2'].iloc[-1] > RSI_OVERBOUGHT
        is_oversold = minute_chart['RSI2'].iloc[-1] < RSI_OVERSOLD
        macd_line = minute_chart['MACD2'].iloc[-1]
        macd_signal_line = minute_chart['MACD_Signal2'].iloc[-1]
        cci = minute_chart['CCI2'].iloc[-1]

        # 롱/숏 포지션 청산
        if agent.is_open():
            if agent.get_is_long():
                # 롱 포지션에 진입해 있다면, 롱 포지션 청산하기
                is_stop_loss_long_position = minute_chart['Close2'].iloc[-1] <= (1 - STOP_LOSS_LONG_POSITIONS) * agent.get_opened_price()
                is_take_profit_long_position = minute_chart['Close2'].iloc[-1] >= (1 + TAKE_PROFIT_LONG_POSITIONS) * agent.get_opened_price()

                if is_stop_loss_long_position:
                    agent.close(minute_chart['Close2'].iloc[-1], date)
                    global long_lose 
                    long_lose += 1
                ...
                is_stop_loss_short_position = minute_chart['Close2'].iloc[-1] >= (1 + STOP_LOSS_SHORT_POSITIONS) * agent.get_opened_price()
                is_take_profit_short_position = minute_chart['Close2'].iloc[-1] <= (1 - TAKE_PROFIT_SHORT_POSITIONS) * agent.get_opened_price()
        ...
        # 롱/숏 포지션 진입
            ...
            if (macd_line > macd_signal_line) and is_oversold and cci > -100:
                agent.open(minute_chart['Close2'].iloc[-1], date, True)
            # 포지션에 진입해있지 않다면, 숏 포지션 진입하기
            if (macd_line < macd_signal_line) and is_overbought and cci < 100:
                agent.open(minute_chart['Close2'].iloc[-1], date, False)

        minute_chart['RoR2'].iloc[-1] = ((agent.get_equity() / agent.get_initial_cash()) - 1) * 100
        ...
        if INITAIL:
            ...
            for dictionary in dict_list:
                ...
                send_message(message, 'raw-data-basic.json')
            producer.flush()
        else:
...
klines = client.futures_klines(symbol=SYMBOL, interval=Client.KLINE_INTERVAL_5MINUTE, start_str=str(datetime(2023, 5, 11, 10)))
